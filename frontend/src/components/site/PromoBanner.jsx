...
export default function PromoBanner() {
  ...
  useEffect(() => {
    try {
      const dismissed = sessionStorage.getItem("kumara_promo_dismissed");
  ...
  return (
    <div data-testid="promo-banner" className="relative z-[60] bg-gradient-to-r from-primary via-[#ff7a2a] to-accent text-primary-foreground">
      <div className="max-w-7xl mx-auto px-5 md:px-8 lg:px-10 py-2.5 flex items-center gap-3 text-xs md:text-sm">
        ...
        <span className="flex-1 truncate font-medium">{t.promo.text}</span>
        <a
          href={whatsappUrl("Halo Kumara, saya tertarik dengan promo pasang baru gratis 1 bulan.")}
          target="_blank"
